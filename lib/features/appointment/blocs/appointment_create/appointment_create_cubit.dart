...
class AppointmentCreateCubit extends Cubit<AppointmentCreateState> {
  ...
  Future<void> updateAppointment({
    ...
    emit(state.copyWith(isLoading: true, isSuccess: false, clearError: true));
    final updatedAppointment = appointment.status == AppointmentStatus.pending
        ? appointment.copyWith(
            clearTenantCancelReason: true,
    ...
    if (trimmedReason.isEmpty) {
      emit(
        state.copyWith(
          isLoading: false,
          isSuccess: false,
          errorMessage: 'Vui lòng nhập lý do hủy lịch hẹn.',
        ),
      );
      return;
    }
    if (trimmedReason.length > 200) {
      emit(
        state.copyWith(
          isLoading: false,
          isSuccess: false,
          errorMessage: 'Lý do hủy lịch hẹn tối đa 200 ký tự.',
        ),
      );
      return;
    }
  ...
  void setExistingAppointment(AppointmentModel? appointment) {
    if (appointment == null || appointment.appointmentId.trim().isEmpty) {
