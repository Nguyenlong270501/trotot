...
List<AppointmentModel> filterHistoryAppointments(
  List<AppointmentModel> items, {
  DateTime? now,
}) {
  final reference = now ?? DateTime.now();
  ...
  final filtered = items
      .where(
        (item) {
          final status = normalizeAppointmentStatus(item.status);
...
List<AppointmentModel> appointmentsForTab(
  ...
  return switch (tab) {
    ...
    AppointmentFeedTab.history => filterHistoryAppointments(items, now: now),
  };
}
