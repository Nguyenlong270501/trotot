...
class FirebaseHomeRemoteDataSource implements HomeRemoteDataSource {
  ...
  Stream<List<PropertyModel>> watchLatestPropertiesByType({
    ...
    final normalizedType = PropertyConstants.normalizePropertyType(
      propertyType,
    );
    var query = _firestore
        .collection('properties')
  ...
  /// Bước 1: lọc property trên Firestore theo minRoomPrice/maxRoomPrice .
  ...
  Query<Map<String, dynamic>> _applyCityFilter(
    ...
    final cityValues = PropertyConstants.firestoreCityAliases(
      city,
    ).take(firestoreInClauseLimit).toList();
...
class _CachedPropertyRooms {
  const _CachedPropertyRooms({required this.updatedAt, required this.rooms});
